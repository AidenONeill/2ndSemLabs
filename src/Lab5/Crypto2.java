...
//		System.out.println("Uknown	|Dan	|Eng	|Fin	|Fre	|Ger	|Ice	|Pol	|Rus	|Spa	|Swe"	);
//		for(int i=0;i<25;i++)
//		{
//			
//			System.out.printf("%.2f",ar[i+1]/one.length()*100);
//			System.out.println("	|"+Dan[i]+"	|"+Eng[i]+"	|"+Fin[i]+"	|"+Fre[i]+"	|"+Ger[i]+"	|"+Ice[i]+"	|"+Pol[i]+"	|"+Rus[i]+"	|"+Spa[i]+"	|"+Swe[i]);
//		}
		print(ar,Dan,"Danish");
	...
	public static double difference(double[] ar,double[] ar2)
	{
		double diff = 0.0;
		for(int i=0; i<25; i++) //go through frequency array
		{
			diff = diff + Math.abs((double)ar[i+1]/one.length()*100-ar2[i]);
		}
		return diff;
	}
	public static void print(double[]ar,double[]ar2, String name)
	{
		...
		System.out.printf("%.2f",100-difference(ar,ar2));
		System.out.print("% chance that unknown is : "+ name);
		System.out.println();
	}
}
